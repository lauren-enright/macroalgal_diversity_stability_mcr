...
taxa_order <- c(focal_taxa, "Other")

aggdat_hab_other <- macro_functional_groups_long %>%
  mutate(
    taxa_grouped = if_else(taxa %in% focal_taxa, taxa, "Other")
  ) %>%
  # Step 1: sum taxa within each plot/sample first
  group_by(site, location, year, habitat, taxa_grouped) %>%
  summarise(
    prop_cover = sum(prop_cover, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  # Step 2: then average across plots/samples
  ...
  mutate(
    taxa_grouped = factor(taxa_grouped, levels = taxa_order),
    habitat = factor(
      habitat,
